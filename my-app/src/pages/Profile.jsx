...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const userEmail = userData ? userData.email.trim().toLowerCase() : ""; // نقوم بتهيئة البريد الإلكتروني ليكون بصيغة منخفضة الحروف وبدون مسافات زائدة باستخدام
    ...
    if (userId) {
        ...
        .then((response) => {
          ...
          if (ordersData) {
            const userOrders = Object.keys(ordersData)
              ...
              .map((key) => ({ id: key, ...ordersData[key] }));/**لف ع كل ال معرفات وطبعلي كل معلوماتو */
      ...
      axios
        ...
        .then((response) => {
          ...
          setLoading(false); // عينها للفولس انو عمليه الجلب نتهت
        })
        .catch((error) => {
