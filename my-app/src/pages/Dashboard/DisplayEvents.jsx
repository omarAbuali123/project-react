  ...
  {/**************fetch data******************* */}
    useEffect(() => {
      const fetchData = async () => {
        try {
          const res = await axios.get('https://tickets-73a3c-default-rtdb.firebaseio.com/events.json');
          setData(res.data);
        } catch (error) {
...
const totalPages = data ? Math.ceil(Object.keys(data).length / itemsPerPage) : 0;
const startIndex = (currentPage - 1) * itemsPerPage;
const endIndex = startIndex + itemsPerPage;
const currentItems = data ? Object.keys(data).slice(startIndex, endIndex) : [];
  {/**********************end pagination****************** */}
  ...
  <div className="w-2/4 py-10">
    ...
    {showSuccessMessage && 
        <div id="alert-3" className="flex items-center p-4 my-20 mb-4 text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400" role="alert">
   ...
   {/*************Add Event*********** */}
                  <button type="button" className="w-32 px-4 py-2 mb-10 font-bold text-white rounded bg-slate-400 hover:bg-slate-300 " onClick={() => { AddEvent(); }} >
                     Add Event +
                  </button> 
  {/************end Add Event******** */}
          ...
          {/****************************** */}
            ...
            {currentItems.map(key => (
                ...
                <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" key={key}>
                    <td className="p-4">
                        <img src={data[key].mainImage}  className="w-16 max-w-full max-h-full md:w-32" alt="Apple Watch"/>
                    </td>
         ...
         <div className="flex justify-center my-4">
                {Array.from({ length: totalPages }, (_, index) => (
                    <button
                        ...
                        className={`px-3 py-1 mx-1 ${currentPage === index + 1 ? 'bg-gray-500 text-white' : 'bg-gray-200 text-gray-700'}`}
                    >
                        {index + 1}
                    </button>
                ))}
